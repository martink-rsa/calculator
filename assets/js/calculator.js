...
// Current issues:
// No button switch functionality in settings
...
function Calculator() {
  const operands = []; // left operand = operands[0], right operand = operands[1]
  let operator = '';
  let lastAction;
  const signs = [['*', '/', 'sqrt'], ['\xD7', '\xF7', '\u221A']];
  const stream = document.getElementById('display-stream');
  const infobar = document.getElementById('display-infobar');
  const calculate = function calculate(n1, op, n2) {
    let result;
    if (op === '+') {
      ...
      result = parseFloat(n1) / parseFloat(n2);
    // R.I.P other operators that were inlcuded in the initial design:
    // %, sqrt, powX(root), pow3(cube)
  ...
  const formatInput = function formatInput(s) {
    // Format flairs added to result
    return s;
  };
  const formatResult = function formatResult(r) {
    return r;
  };
  const displayResult = function displayResult(r) {
  ...
  function displayInfobar(sliceAmount, ...args) {
  ...
  function setEvents() {
    const buttons = document.getElementsByClassName('button-main');
...
function CalculatorUI() {
  ...
  function setEventActions() {
    ...
    // Borked. The code below is meant to close the menu
    //      if area outside the settings/options window
    //      is clicked, but this isn't working as intended.
    //      Clicks are being registered when clicking
  ...
  function beginAnimation() {
    let start = null;
    const element = document.getElementById('main-fx-overlay2');
    const degrees = [201, 192, 48, 65, 4, 228, 163, 152, 302, 90];
